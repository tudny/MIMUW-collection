...
const char *path_intersection(const char **A, const char **B) {
    const char *a = *A;
    const char *b = *B;
    ...
    // Załóżmy, że |a| >= |b|
    if (a_len < b_len) {
        return path_intersection(B, A);
    }

    char *intersection = calloc(sizeof(char), b_len + 1);
    char *to_return = intersection;
    ...
    *A += (intersection - to_return);
    *B += (intersection - to_return);
