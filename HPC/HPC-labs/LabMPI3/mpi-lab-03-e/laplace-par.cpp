...
#define SEND_FST_TAG 42
#define SEND_LST_TAG 43
...
static std::tuple<int, double> performAlgorithm(
  int myRank, int numProcesses, GridFragment *frag, double omega, double epsilon) {
    ...
    double maxDiff = 0;
    double maxDiffFromAll = 0;
    int numIterations = 0;

    MPI_Request requests[2][2];
    MPI_Request requests2[2][2];
    MPI_Status statuses[2][2];
    ...
    do {
        ...
        for (int color = 0; color < 2; ++color) {
            ...
            int negColor = 1 - color;

            MPI_Isend(
                frag->data[negColor][1],
                frag->gridDimension / 2,
                MPI_DOUBLE,
                (myRank - 1 + numProcesses) % numProcesses,
                SEND_FST_TAG,
                MPI_COMM_WORLD,
                &requests[negColor][0]
            );
            MPI_Isend(
                frag->data[negColor][frag->lastRowIdxExcl - frag->firstRowIdxIncl],
                frag->gridDimension / 2,
                MPI_DOUBLE,
                (myRank + 1) % numProcesses,
                SEND_LST_TAG,
                MPI_COMM_WORLD,
                &requests[negColor][1]
            );
            ...
            MPI_Irecv(
                frag->data[negColor][frag->lastRowIdxExcl - frag->firstRowIdxIncl + 1],
                frag->gridDimension / 2,
                MPI_DOUBLE,
                (myRank + 1) % numProcesses,
                SEND_FST_TAG,
                MPI_COMM_WORLD,
                &requests2[negColor][0]
            );
            MPI_Irecv(
                frag->data[negColor][0],
                frag->gridDimension / 2,
                MPI_DOUBLE,
                (myRank - 1 + numProcesses) % numProcesses,
                SEND_LST_TAG,
                MPI_COMM_WORLD,
                &requests2[negColor][1]
            );

            MPI_Waitall(2, requests[negColor], statuses[negColor]);
            MPI_Waitall(2, requests2[negColor], statuses[negColor]);
        ...
        MPI_Allreduce(&maxDiff, &maxDiffFromAll, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);

        ++numIterations;
    } while (maxDiffFromAll > epsilon);
    /* no code changes beyond this point should be needed */
    return std::make_tuple(numIterations, maxDiffFromAll);
}
