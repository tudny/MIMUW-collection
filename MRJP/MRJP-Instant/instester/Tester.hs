...
testJvmOne classpath fp = do
  ...
  ft <- toTextWarn fp
  cmd insc ft
  let filename = basename fp
  let dir = dirname fp
  filenameText <- toTextWarn filename
  let expectedOutputPath = dir </> filename <.> "output"
  expectedOutput <- toTextWarn expectedOutputPath
  cmd "java" "-cp" classpath filenameText -|- cmd "diff" "-"  expectedOutput
  return True
...
testLlvmOne fp = do
  ...
  bcnameText <- toTextWarn bcname
  let expectedOutputPath = dir </> filename <.> "output"
  expectedOutput <- toTextWarn expectedOutputPath
  cmd "lli" bcnameText -|- cmd "diff" "-"  expectedOutput
  return True
